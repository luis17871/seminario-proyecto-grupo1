...
with tab1:
    ...
    # SECCION DE EJECUCION DEL PIPELINE
    st.markdown("---")
    st.markdown("<h3 class='animate-in'>Pipeline de Procesamiento de Datos</h3>", unsafe_allow_html=True)
    st.markdown("Ejecuta el pipeline completo para procesar los datos desde cero y generar las metricas RFM.")
    
    col_btn1, col_btn2, col_btn3 = st.columns([2, 1, 2])
    
    with col_btn2:
        ejecutar_pipeline = st.button("EJECUTAR PIPELINE COMPLETO", type="primary", use_container_width=True)
    ...
    if ejecutar_pipeline:
        ...
        try:
            ...
            # Paso 1: Cargar datos
            with col_msg2:
                status_container.info("📂 [1/4] Cargando datos desde BASERETAIL.csv...")
                progress_bar.progress(10)
            
            ruta_datos = os.path.join(BASE_DIR, "data", "BASERETAIL.csv")
            df = cargar_datos(ruta_datos)
            
            if df is None or df.empty:
                with col_msg2:
                    status_container.error("❌ No se pudieron cargar los datos. Verifica que el archivo exista.")
            else:
                with col_msg2:
                    status_container.success(f"✅ [1/4] Datos cargados: {len(df):,} registros")
                    progress_bar.progress(25)
                
                # Paso 2: Limpieza de datos
                with col_msg2:
                    status_container.info("🧹 [2/4] Aplicando limpieza de datos...")
                    progress_bar.progress(40)
                
                df_limpio = limpiar_datos(df)
                
                with col_msg2:
                    status_container.success(f"✅ [2/4] Datos limpiados: {len(df_limpio):,} registros")
                    progress_bar.progress(55)
                
                # Paso 3: Generar metricas RFM
                with col_msg2:
                    status_container.info("📊 [3/4] Generando metricas RFM (Recency, Frequency, Monetary)...")
                    progress_bar.progress(70)
                
                df_features = generar_indicadores_rfm(df_limpio)
                
                with col_msg2:
                    status_container.success(f"✅ [3/4] Metricas RFM generadas: {len(df_features):,} clientes")
                    progress_bar.progress(85)
                
                # Paso 4: Guardar resultados
                with col_msg2:
                    status_container.info("💾 [4/4] Guardando resultados...")
                    progress_bar.progress(90)
                
                ruta_salida = os.path.join(BASE_DIR, "data", "processed", "clientes_limpios.csv")
                ruta_salida_ml = os.path.join(BASE_DIR, "data", "processed", "metricas_clientes_ml.csv")
                ...
                with col_msg2:
                    ...
                    status_container.success("PIPELINE COMPLETADO CON EXITO")
                
                # Mostrar resultados fuera del contenedor de mensajes
                st.markdown("---")
                ...
                with col_res2:
                    st.metric("Registros Limpios", f"{len(df_limpio):,}")
                with col_res3:
                    st.metric("Clientes Unicos", f"{len(df_features):,}")
                
                st.info("Los datos se han actualizado. Recarga la pagina para ver los cambios en los graficos.")
                
                # Boton para recargar centrado
                col_reload1, col_reload2, col_reload3 = st.columns([2, 1, 2])
                with col_reload2:
                    if st.button("Recargar Dashboard", type="secondary", use_container_width=True):
                        st.rerun()
                    
        except Exception as e:
            with col_msg2:
                status_container.error(f"Error durante la ejecucion del pipeline: {str(e)}")
...
with tab2:
    ...
    if api_activa:
        st.success(f"✅ API de Segmentacion Conectada URL: `{API_BASE_URL}`")
    else:
